...
var (
	addr = flag.String("addr", "localhost:50051", "the address toconnect to")
)
...
func main() {
	...
	r.GET("/movies", func(ctx *gin.Context) {
		...
		ctx.JSON(http.StatusOK, gin.H{"movies": res.Movies})
	})
	r.GET("/movies/:id", func(ctx *gin.Context) {
		...
		ctx.JSON(http.StatusOK, gin.H{"movie": res.Movie})
	})
	r.POST("/movies", func(ctx *gin.Context) {
		...
		res, err := client.CreateMovie(ctx, &pb.CreateMovieRequest{Movie: data})
		if err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": err})
			return
		}
		ctx.JSON(http.StatusCreated, gin.H{"movie": res.Movie})
	})
	r.PUT("/movies/:id", func(ctx *gin.Context) {
		...
		res, err := client.UpdateMovie(ctx, &pb.UpdateMovieRequest{Movie: &pb.Movie{
			Id:    movie.ID,
			Title: movie.Title,
			Genre: movie.Genre,
		}})
